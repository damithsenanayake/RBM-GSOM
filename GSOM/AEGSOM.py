...
class GSOM(object):
    ...
    def train_batch(self, X, iterations, lr, prune):
        ...
        for i in range(iterations):
            ...
            if prune:
                for k in self.grid.keys():
                    if  len(self.hits.values()) > self.min_nodes and ( self.hits[k] < 1 or self.gen[k] < self.current_gen * 0.5  * np.exp(-i / iterations)) :#and len(self.grid) > or self.gen[k] < self.current_gen * 0.75 self.min_nodes   :
                        del self.gen[k]
                        del self.errors[k]
                        del self.learners[k]
                        del self.grid[k]
