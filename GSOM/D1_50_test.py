...
gsom = GSOM(dims=6,  hid=2, fd=0.8, sf=0.75, radius=50, max_nodes=3000, min_nodes=10, gaussian=True, nei=True, X=X_)

for i in range(500):

    print "\n iteration ", (i+1)
    gsom.train_batch(X_, 1, 0.1*np.exp(-i/10), False)
...
plt.scatter(x, y, edgecolors='none', c = np.array(colors)[Y] )
plt.show()
